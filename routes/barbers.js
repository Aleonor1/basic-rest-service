...
const Barbers = require('../classes/Barbers');
...
router.post('/', (req, res) => {
    ...
    if (!arrayOfBarbers.isIdInArray(data.id)) {
        arrayOfBarbers.newBarber(data.firstName, data.lastName, data.id, data.address, data.phone);
...
router.put('/:id', (req, res) => {
    ...
    if (arrayOfBarbers.isIdInArray(id)) {
        let barber = new Barber(data.firstName, data.lastName, data.id, data.address, data.phone);
...
router.get('/:id/hasVacation', (req, res) => {
    ...
    if (barber != null) {
        ...
        if (barber.hasVacation()) {
            res.send(`Barber with id ${req.params.id} has vacation on ${barber.freeDays}`);
        } else {
